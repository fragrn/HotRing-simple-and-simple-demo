...
public:
    HashTableClient(ht &kvStore, int hotspot_ratio)
    ...
    void performOperations() {
        cout << "The performance of HashTable without hotspot shift" << endl;
...
private:
    ...
    void read() {
        string key = chooseKey();
        htEntry *result = kvStore.search(key);
        if (result != nullptr) {
            // 操作成功，返回值
...
int main() {
    srand(time(0));  // 设置随机种子
    ht hashTable(1024);  // 1024 为哈希表的大小

    int hotspot_ratio;
    cout << "Enter the hotspot ratio (in percentage): ";
    cin >> hotspot_ratio;

    HashTableClient client(hashTable, hotspot_ratio);
